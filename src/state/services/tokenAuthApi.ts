...
const injectedRtkApi = api.injectEndpoints({
  endpoints: build => ({
    ...
    apiTokenauthValidatetokenPost: build.mutation<
      ApiTokenauthValidatetokenPostApiResponse,
      ApiTokenauthValidatetokenPostApiArg
    >({
      query: queryArg => ({
        url: `/api/TokenAuth/ValidateToken`,
        method: 'POST',
        body: queryArg.tokenVerificationModel,
      }),
    }),
    ...
    >({
      query: () => ({url: `/api/TokenAuth/GetExternalAuthenticationProviders`}),
    }),
...
export type ApiTokenauthValidatetokenPostApiResponse =
  /** status 200 Success */ boolean;
export type ApiTokenauthValidatetokenPostApiArg = {
  tokenVerificationModel: TokenVerificationModel;
};
...
export type TokenVerificationModel = {
  userNameOrEmailAddress: string;
  token: string;
  tenancyName?: string | null;
};
...
export const {
  ...
  useApiTokenauthValidatetokenPostMutation,
